				...
				if curSong ~= nil then
					...
					if path ~= nil then
						...
						if startFromPreview then -- When starting from preview point
							SOUND:PlayMusicPart(path,curSong:GetSampleStart(),song:GetLastSecond()-curSong:GetSampleStart(),2,2,loop,true,true);
							if themeConfig:get_data().global.SongPreview == 3 then 
								startFromPreview = false
							end
							start = 0
						else -- When starting from start of from exit point.
							SOUND:PlayMusicPart(path,start,song:GetLastSecond()-start,2,2,false,true,true);
