...
local LaneCoverOptions= {
	...
	nesty_options.float_config_val(playerConfig, "LaneCoverLayer", 1, 1, 2, notefield_draw_order.under_explosions,notefield_draw_order.over_field),
}
...
local gameplay_options= {
	nesty_options.float_config_val(playerConfig, "ScreenFilter", -2, -1, -1, 0, 1),
...
local item_params= {
	text_commands= {
		Font= "Common Normal", OnCommand= function(self)
			self:diffusealpha(0):smooth(0.5):diffusealpha(1)
		end,
	},
	text_width= .7,
	value_text_commands= {
...
for pn, menu in pairs(menus) do
	...
	frame[#frame+1]= menu:create_actors{
		...
		display_params= {
			...
			on= function(self)
				--self:zoomy(0):smooth(0.3):zoomy(1)
			end},
	}
