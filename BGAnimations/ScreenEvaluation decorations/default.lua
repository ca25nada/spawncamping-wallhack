...
local function GraphDisplay( pn )
	...
	local t = Def.ActorFrame {
		...
		LoadFont("Common Normal")..{
			InitCommand=cmd(xy,WideScale(get43size(140),140),-35;zoom,0.4;halign,1;valign,0;diffusealpha,0.7;);
			BeginCommand=function(self) 
				...
				if GAMESTATE:GetNumPlayersEnabled() == 1 and GAMESTATE:IsPlayerEnabled(PLAYER_2)then
					self:x(-(SCREEN_CENTER_X*1.65)+(SCREEN_CENTER_X*0.35)+WideScale(get43size(140),140))
				end;
				if GAMESTATE:GetNumPlayersEnabled() == 2 and pn == PLAYER_2 then
					self:x(SCREEN_CENTER_X*0.30)
					self:halign(1)
		...
		LoadFont("Common Normal")..{
			InitCommand=cmd(xy,WideScale(get43size(140),140),25;zoom,0.4;halign,1;valign,0;diffusealpha,0.7;);
			BeginCommand=function(self) 
