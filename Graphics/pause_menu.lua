...
local function create_menu_item(pn, x, y, item_name)
	...
	return Def.ActorFrame{
		...
		Def.Quad{
			Name="Quad",
			InitCommand = function(self)
				self:zoomto(menu_bg_width,menu_spacing)
...
local function show_menu(pn)
	...
	for i, item in ipairs(menu_items[pn]) do
		item:GetChild("Text"):playcommand("LoseFocus")
		item:GetChild("Quad"):playcommand("LoseFocus")
	end
	current_menu_choice[pn]= 1
	menu_items[pn][current_menu_choice[pn]]:GetChild("Text"):playcommand("GainFocus")
	menu_items[pn][current_menu_choice[pn]]:GetChild("Quad"):playcommand("GainFocus")
